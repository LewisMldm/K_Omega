...
Func3 = (de*inner(u, grad(w))*s*dx - (alpha1*F1(k, w, y) 
	+ alpha2*(1 - F2(k, w, y)))*w*inner(Tau(k, w, u), StrT(u))*s*dx
...
for foo in range(100):
	try:
		...
		pk_val = Function(P1_scalar).interpolate(Pk(u, k, w))
		print(f"DEBUG: Min Pk after error: {pk_val.dat.data.min()}, Max Pk after error: {pk_val.dat.data.max()}")
