...
Ubdry = Function(V).interpolate(as_vector([0, Vent*((-4/9)*(x-7)*(x-10))]))
bcs = [DirichletBC(Z.sub(0), Ubdry, (23,)),
       DirichletBC(Z.sub(0), Constant((0, 0)), (19, 21,22))]
solve(a==0, up, bcs=bcs)

pol = (Diff_coef*inner(grad(t),grad(l)) + dot(u, grad(t))*l)*dx - 100*exp(-10 * ((x - 8.5)**2 + (y - 0)**2))*l*dx
bcp = DirichletBC(T, Constant(0), (20))
...
J = assemble(conditional(le(x,7), t**2, Constant(0)) * dx) + 0.1*assemble(Vent**2*ds(23))
#Jhat = ReducedFunctional(J, [Control(JetIn)])
Jhat = ReducedFunctional(J, [Control(Vent)])
stop_annotating()

u_, p_ = up.subfunctions
u_.rename("Mean Velocity")
p_.rename("Pressure")
File = VTKFile("Stokes_opt/Vent.pvd")
File.write(u_, p_, t)

print(f"Vent vel = {float(Vent):1.2f}, J = {float(Jhat(Vent)):1.2e}")
...
Ubdry.interpolate(as_vector([0, Vent*((-4/9)*(x-7)*(x-10))]))
solve(a==0, up, bcs=bcs)
solve(pol==0, t, bcs=bcp)
File.write(u_, p_, t)
